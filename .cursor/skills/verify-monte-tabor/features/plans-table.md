...
Three prepaid funeral-plan cards — Económico, Jardín, and Presidencial — show monthly per-collaborator prices, a featured Jardín tab, shared benefits, and an IVA footnote.

## Sub-features

- `plans-heading` names the section.
- `plans-columns` shows the three plan prices and the Jardín featured tab.
- `plans-rows` shows coffin type plus included-service ticks.
...
- Drive phone-portrait (`hand`) and tablet-portrait (`lap`) first. Desk-band proof (`tablet-landscape` or `desktop`) is additional, not a substitute.
...
- **Differentiator.** Read coffin types. Each article’s `Tipo de ataúd` value matches that plan’s name: `Económico`, `Jardín`, `Presidencial`.
